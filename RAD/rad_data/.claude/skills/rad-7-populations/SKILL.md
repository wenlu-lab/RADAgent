---
name: rad-7-populations
description: Run RAD pipeline Step 7 (Stacks populations) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# RAD Pipeline — Step 7: Stacks populations (Autonomous)

You are running RAD pipeline Step 7 directly on the server. All paths are relative to the project root (`rad_data/`). This skill is **fully autonomous** — it launches populations, monitors with adaptive polling, and verifies output.
...
1. **gstacks output exists**: `test -f 05-stacks/catalog.fa.gz && test -f 05-stacks/catalog.calls && echo OK`
   - FAIL → **stop**: "Step 6 not completed. Run `/rad-6-gstacks` first."
...
| "unable to open catalog" | gstacks output missing/corrupt | Re-run `/rad-6-gstacks` |
