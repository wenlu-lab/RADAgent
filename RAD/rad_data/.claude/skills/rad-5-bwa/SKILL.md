---
name: rad-5-bwa
description: Run RAD pipeline Step 5 (BWA alignment) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# RAD Pipeline — Step 5: BWA Alignment (Autonomous)

You are running RAD pipeline Step 5 directly on the server. All paths are relative to the project root (`rad_data/`). This skill is **fully autonomous** — once invoked, it handles indexing, alignment, monitoring, and verification without user intervention.
...
1. **Samples exist**: `ls 04-all_samples/*.1.fq.gz | wc -l` → EXPECTED_SAMPLES. `ls 04-all_samples/*.2.fq.gz | wc -l` — must match
   - FAIL → **stop**: "Step 3 not completed. Run `/rad-3-rename` first."
