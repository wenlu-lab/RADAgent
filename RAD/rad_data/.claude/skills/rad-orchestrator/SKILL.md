...
description: Run the complete 16-step RAD pipeline (steps 0-15) sequentially. Each step has 4 layers of error defense: step self-heal, step retry, orchestrator retry, and autonomous debugger. Use when user says "run full pipeline", "run all steps", "orchestrate pipeline", or "resume from step N". Invoke with /rad-orchestrator [start-step] [--clean].
argument-hint: [start-step] [--clean] [--only]
allowed-tools: Bash, Read, Grep, Glob, Skill, Write
---

# RAD Pipeline — Master Orchestrator

Run all 16 steps of the RAD bioinformatics pipeline in sequence. Each step is executed by invoking its dedicated skill (`/rad-0-lane-info` through `/rad-15-even-dist`) as a forked subagent. This orchestrator stays in the main conversation to coordinate, log, and handle failures. Token tracking is handled by `00-scripts/rad_token_tracker.py` and written to `rad-pipeline-token-report.txt` (not displayed to user).
...
Examples:
- `/rad-orchestrator` — full pipeline from step 0 through 15
- `/rad-orchestrator 5` — resume from step 5 through 15
- `/rad-orchestrator 5 --clean` — clean steps 5-15, then run from step 5
- `/rad-orchestrator --clean` — clean all, then run from step 0
- `/rad-orchestrator 0 --only` — run ONLY step 0
- `/rad-orchestrator 8 --only --clean` — clean step 8, run only step 8

## Step Registry

| Step | Skill | Title | Est. Time |
|------|-------|-------|-----------|
| 0 | rad-0-lane-info | Prepare Lane Info | 5s |
| 1 | rad-1-cutadapt | Cutadapt Adapter Trimming | 10min |
| 2 | rad-2-radtags | process_radtags Enzyme Filtering | 30min |
| 3 | rad-3-rename | Rename Samples | 10s |
| 4 | rad-4-popmap | Prepare Population Map | 1s |
| 5 | rad-5-bwa | BWA Alignment | 1-2hr |
| 6 | rad-6-gstacks | gstacks Genotyping | 30-60min |
| 7 | rad-7-populations | Stacks populations | 10-30min |
| 8 | rad-8-vcf-filter | VCF Filtering + Chr Selection | 1min |
| 9 | rad-9-snp-dup-hwe | SNP Duplication + HWE | 2min |
| 10 | rad-10-ld-clump | LD Clumping | 5min |
| 11 | rad-11-remove-atgc | Remove A/T and G/C SNPs | 5s |
| 12 | rad-12-maf-filter | MAF 0.1 Filter | 5s |
| 13 | rad-13-flanking | Flanking Variants Filter | 10s |
| 14 | rad-14-blast-map | BLAST Mapping | 2min |
| 15 | rad-15-even-dist | Even Distribution (Final Panel) | 5s |

---

## Phase 1: Pre-flight

1. **Verify project root**: Run `test -d 02-raw && test -d 00-scripts && echo OK`. If FAIL, try `cd ~/rad_data` and recheck. If still FAIL → stop: "Not in rad_data directory."
...
3. **Record pipeline start time**:
   ```bash
   date +%s > /tmp/rad_pipeline_start.txt
   ```

4. **Initialize timing tracker**:
   ```bash
   python3 00-scripts/rad_token_tracker.py --init
   ```

5. **Initialize log**:
   ```bash
   echo "" >> rad-pipeline.log
   echo "================================================" >> rad-pipeline.log
   echo "RAD Pipeline Run — $(date '+%Y-%m-%d %H:%M:%S')" >> rad-pipeline.log
   echo "Starting from step: <START_STEP>" >> rad-pipeline.log
   echo "Clean mode: <yes/no>" >> rad-pipeline.log
   echo "================================================" >> rad-pipeline.log
   ```

6. **Report to user**: "Starting RAD pipeline from step START_STEP. Clean mode: yes/no."
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] CLEAN: Removed outputs for steps <START_STEP>-15" >> rad-pipeline.log
```
...
```bash
date +%s > /tmp/rad_step_start.txt
echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — STARTING" >> rad-pipeline.log
```
...
Use the Skill tool:
```
skill: rad-<N>-<name>  (from the Step Registry table above, e.g., step 0 → rad-0-lane-info, step 5 → rad-5-bwa)
```
...
```bash
STEP_START=$(cat /tmp/rad_step_start.txt)
STEP_END=$(date +%s)
DURATION=$((STEP_END - STEP_START))

# Record step timing + tokens to CSV (token tracker handles extraction)
python3 00-scripts/rad_token_tracker.py --record-step <N> "<Title>" <STATUS> $STEP_START $STEP_END

echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — <STATUS> (${DURATION}s)" >> rad-pipeline.log
```
...
1. Log the first failure:
   ```bash
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — FAILED (attempt 1), retrying..." >> rad-pipeline.log
   ```
...
4. Parse the retry result the same way (3c: parse status, 3d: record timing + tokens + log).

5. **If retry succeeds**: Log as SUCCESS with note "(passed on retry)", continue to next step.

6. **If retry also fails → ESCALATE TO DEBUGGER (Layer 4):**

   Log the escalation:
   ```bash
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — FAILED (attempt 2), escalating to debugger..." >> rad-pipeline.log
   ```
   ...
   ```bash
   date +%s > /tmp/rad_debugger_start.txt
   ```
   ...
   ```bash
   DBG_START=$(cat /tmp/rad_debugger_start.txt)
   DBG_END=$(date +%s)
   DBG_DURATION=$((DBG_END - DBG_START))
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] DEBUGGER: Step <N> analysis completed in ${DBG_DURATION}s" >> rad-pipeline.log
   ```
   ...
   **7a. If FIX_APPLIED → RE-INVOKE THE STEP (attempt 3):**
   ...
   ```bash
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — Debugger applied fix, re-running step (attempt 3)..." >> rad-pipeline.log
   ```
   ...
   ```
   skill: rad-<N>-<name>  (from the Step Registry table above)
   ```

   Parse result the same way (3c: parse status, 3d: record timing + tokens + log).

   If SUCCESS:
   ```bash
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — SUCCESS (passed after debugger fix, attempt 3)" >> rad-pipeline.log
   ```
   ...
   **7b. If UNFIXABLE or attempt 3 FAILURE → STOP for real:**
   ...
   ```bash
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP <N>: <Title> — FAILED (all 4 layers exhausted), stopping pipeline" >> rad-pipeline.log
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] PIPELINE STOPPED — Step <N> failed after retry + debugger" >> rad-pipeline.log
   echo "[$(date '+%Y-%m-%d %H:%M:%S')] Resume with: /rad-orchestrator <N>" >> rad-pipeline.log
   ```
   ...
   - Resume command: `/rad-orchestrator <N>` or `/rad-orchestrator <N> --clean`
...
```bash
PIPELINE_START=$(cat /tmp/rad_pipeline_start.txt)
PIPELINE_END=$(date +%s)
...
**Log completion:**
```bash
echo "[$(date '+%Y-%m-%d %H:%M:%S')] PIPELINE <COMPLETED|STOPPED> — Total time: ${TOTAL_DURATION}s" >> rad-pipeline.log
```

**Display the summary** (focused on step execution and outcomes):

```
## RAD Pipeline Summary
...
Build this table from `rad-pipeline-timing.csv` (columns: STEP, TITLE, STATUS, DURATION_SEC). Convert duration to human-readable (Xs, Xm Ys, Xh Ym Zs). The Debugger column shows `N/A` for steps where the debugger was not invoked, and `FIX_APPLIED`/`UNFIXABLE` if it was.
...
```bash
python3 00-scripts/rad_token_tracker.py --summary > rad-pipeline-token-report.txt
```
...
Total time: <Xh Ym Zs>
Token report: rad-pipeline-token-report.txt
Log file: rad-pipeline.log
```
...
Resume with: /rad-orchestrator <N>
Clean and retry: /rad-orchestrator <N> --clean
```
