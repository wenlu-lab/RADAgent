---
name: rad-14-blast-map
description: Run RAD pipeline Step 14 (SNP mapping against reference genome via BLAST) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# RAD Pipeline — Step 14: SNP Mapping Against Reference Genome

You are running RAD pipeline Step 14 directly on the server. All paths are relative to the project root (`rad_data/`).
...
1. **Input VCF exists**: `test -f filtered_flanking.vcf && echo OK`. Count: `grep -cv '^#' filtered_flanking.vcf` → INPUT_SNPS
   - FAIL → **stop**: "Step 13 not completed. Run `/rad-13-flanking` first."
