---
name: rad-1-cutadapt
description: Run RAD pipeline Step 1 (cutadapt paired-end adapter trimming) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# RAD Pipeline — Step 1: Cutadapt Paired-End Adapter Trimming

You are running RAD pipeline Step 1 directly on the server using **96 CPUs**. All paths are relative to the project root (`rad_data/`).
...
```
bash 00-scripts/01_cutadapt_PE.sh 96
```

This is a **long-running command** processing EXPECTED_SAMPLES paired-end samples in parallel. Capture both stdout and stderr. The script appends all cutadapt output to `10-log_files/{TIMESTAMP}_01_cutadapt.log`.
