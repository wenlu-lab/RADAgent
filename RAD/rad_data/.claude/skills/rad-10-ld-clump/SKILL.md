---
name: rad-10-ld-clump
description: Run RAD pipeline Step 10 (LD clumping) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# RAD Pipeline — Step 10: LD Clumping

You are running RAD pipeline Step 10 directly on the server. All paths are relative to the project root (`rad_data/`).
...
1. **Input VCF exists**: `test -f filtered_m4_p80_x0_S1_chr.singleton.hwe.recode.vcf && echo OK`. Count SNPs: `grep -cv '^#' filtered_m4_p80_x0_S1_chr.singleton.hwe.recode.vcf` → INPUT_SNPS
   - FAIL → **stop**: "Step 9 not completed. Run `/rad-9-snp-dup-hwe` first."
