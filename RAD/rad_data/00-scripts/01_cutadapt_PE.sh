...
# Paired-end mode
ls -1 02-raw/*_1.fastq.gz | 
        sed 's/_1\.fastq\.gz//g' | 
        sed 's/02\-raw\///g' |
parallel -j $NCPU cutadapt -a file:01-info_files/adapters.fasta \
        -A file:01-info_files/adapters.fasta \
        -o 02-raw/trimmed/{}"_R1.fastq.gz" \
        -p 02-raw/trimmed/{}"_R2.fastq.gz" \
        -e 0.2 \
        -m 50 \
        "02-raw/"{}"_1.fastq.gz" "02-raw/"{}"_2.fastq.gz" '2>&1' '>>' 10-log_files/"$TIMESTAMP"_01_cutadapt.log
