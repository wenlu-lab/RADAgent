...
class Skill:
    ...
    model_alias: str = "phi-4"       # short model label recorded in transcripts
...
def build_system_prompt(skill: Skill, tool_descriptions: dict[str, str]) -> str:
    ...
    if allowed_with_desc:
        ...
        parts.extend([
            ...
            "- NEVER prepend an absolute path or `cd` to a different directory. If the skill body mentions `rad_data/` as the project name, that is the historical project name — your cwd is already the right project root, so `rad_data/foo` means just `foo` for you.",
