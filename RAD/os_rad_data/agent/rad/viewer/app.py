"""Textual TUI for browsing RAD runs. A thin view over rad.viewer.model."""
...
class RADViewer(App):
    ...
    def __init__(self, rad_dir, *, live=True, session_id=None):
        super().__init__()
        self.rad_dir = Path(rad_dir)
    ...
    def _reload(self, initial=False) -> None:
        ...
        self.runs = model.load_runs(self.rad_dir)
        troot = self.rad_dir / "transcripts"
        ...
        if not self.runs:
            self.query_one("#detail", Static).update(Text("No runs found under .rad/transcripts/"))
            self.cur_run = None
            return
        idx = 0
