---
name: rad-13-flanking
description: Run RAD pipeline Step 13 (flanking variants filter) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# RAD Pipeline — Step 13: Flanking Variants Filter

You are running RAD pipeline Step 13 directly on the server. All paths are relative to the project root (`rad_data/`).
...
1. **Input VCF exists**: `test -f filtered_m4_p80_x0_S1_chr.singleton.hwe.maf10.LD.no_AT_CG.recode.vcf && echo OK`. Count SNPs: `grep -cv '^#' filtered_m4_p80_x0_S1_chr.singleton.hwe.maf10.LD.no_AT_CG.recode.vcf` → INPUT_SNPS
   - FAIL → **stop**: "Step 12 not completed. Run `/rad-12-maf-filter` first."

2. **Reference VCF exists**: `test -f filtered_m4_p80_x0_S1_chr.recode.vcf && echo OK`
   - FAIL → **stop**: "Reference VCF from Step 8 missing. Run `/rad-8-vcf-filter` first."
