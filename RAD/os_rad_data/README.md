# RAD Pipeline — Self-Hosted Agentic Restriction-site Associated DNA sequencing

An end-to-end RAD (Restriction-site Associated DNA sequencing) bioinformatics pipeline that takes raw
...
runtime (`agent/`) that loads a **self-hosted Phi-4 model in-process via Hugging
...
| Component | Value |
|---|---|
| Model | `microsoft/Phi-4` (bf16, ~28 GB in VRAM) — see note on the FP8 variant below |
| Inference | **in-process** Hugging Face `transformers` (no server); tool calls parsed by `rad/coder_tool_parser.py` |
...
| Runtime | the `rad-agent` package in `agent/`, invoked via the `./rad` CLI |
| GPU | 1× NVIDIA A100 80GB / H100 80GB (≥30 GB VRAM) |
...
> `RAD_MODEL_ID=microsoft/Phi-4`.
>
> **Effective context window:** in-process `transformers` has no PagedAttention, so the
> usable window is much smaller than vLLM's 262K (default `RAD_MAX_CONTEXT_TOKENS=32768`,
...
[orchestrator: one Python process, model loaded in-process]
   ...
   │   • writes timing/status to rad-pipeline.log
   │   • decides retries / invokes the debugger
   │
   └── Skill("rad-5-bwa") ── in-process run_skill() ─▶ [step subagent, fresh-context messages]
                                                                │  Bash / Read / Grep / Glob
                                                                ▼
                                                           bwa mem … | samtools …
```

Every orchestrator and subagent conversation is recorded as JSONL under `.rad/` — which
is exactly what the **`./rad view`** TUI reads (see [Viewing runs](#viewing-runs-rad-view)).
...
### What's recorded under `.rad/`

```
.rad/
├── orchestrator-<YYYYMMDD-HHMMSS>.log   # one per run: "Session: <id>" + final status table
└── transcripts/<session_id>/
    ├── rad-orchestrator.jsonl           # the orchestrator's transcript
    └── subagents/agent-<id>.jsonl       # one per step / debugger invocation
```

One session directory = one `./rad orchestrator` run. Retried steps appear as repeated
...
4. **Autonomous debugger** — if all else fails, `rad-debugger` reads the failed step's
...
| `rad-pipeline.log` | Execution timeline with real timestamps + per-step status |
| `rad-pipeline-timing.csv` | Per-step duration + token usage (latest run) |
| `rad-pipeline-token-report.txt` | Markdown token/timing report (latest run) |
...
- **Chromosome names** — the chromosome filter in `.claude/skills/rad-8-vcf-filter/SKILL.md`.
...
| Symptom | Cause | Fix |
|---|---|---|
| First run pauses minutes before any output | The ~28 GB model is loading into VRAM (one-time per process) | Normal — watch `nvidia-smi`; it stays resident after |
| `RuntimeError: NVIDIA driver too old` | torch built for newer CUDA than the driver | Use the cu118 torch wheels (Installation §3) |
| CUDA out of memory during a long step | Context too large for in-process KV cache (no PagedAttention) | Lower `RAD_MAX_CONTEXT_TOKENS` (e.g. `export RAD_MAX_CONTEXT_TOKENS=24576`) and re-run |
| `FP8 quantized models is only supported on ... compute capability >= 8.9` | You pointed `RAD_MODEL_ID` at the `-FP8` checkpoint on an Ampere GPU (A100 = 8.0) | Use the default bf16 checkpoint (`microsoft/Phi-4`); FP8 only loads on 4090/H100 |
| `./rad status` shows little GPU free before a run | Another process is holding VRAM | Free the card; the model needs ~28 GB contiguous |
| A step reports FAILURE but output files exist | The model judged its own work poorly | Check the files; resume with `./rad orchestrator <next-step>` |
| Log shows fake timestamps like "2023-05-15…" | Model copied a SKILL.md template literally | The real timestamps are in `rad-pipeline.log` / the `.rad` transcripts |
...
├── agent/               # rad-agent: the Python runtime + ./rad CLI (see agent/README.md)
│   └── rad/viewer/      #   the ./rad view TUI (model.py = parser, app.py = Textual UI)
...
├── .rad/                # generated: run logs + JSONL transcripts (what ./rad view reads)
