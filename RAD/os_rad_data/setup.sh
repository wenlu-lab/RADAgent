#!/usr/bin/env bash
# setup.sh — one-shot setup for the open-source RAD pipeline (in-process Phi-4).
#
...
#   4. Pre-download the Phi-4 model (bf16, ~28 GB)
#   5. Print next steps
#
...
if [ "${GPU_FREE_MB:-0}" -lt 70000 ]; then
    red "GPU has only ${GPU_FREE_MB} MB total — Phi-4 bf16 needs ≥30 GB."
...
if [ "${DISK_FREE_GB:-0}" -lt 65 ]; then
    yellow "WARNING: only ${DISK_FREE_GB} GB free in \$HOME — bf16 model needs ~28 GB."
fi
...
# Quick import check
.venv/bin/python -c "import torch, transformers, accelerate, rad.config; print('  imports OK')"
green "All Python packages OK"

# .rad/ holds the agent's transcripts + runtime.log (see agent/rad/config.py);
# the next-steps below also redirect orchestrator logs into it.
if [ ! -d .rad ]; then
    mkdir -p .rad
    green "Created .rad/ (runtime transcripts + logs)"
else
    yellow "Reusing existing .rad/"
fi

# ---------- 4. model download ----------
section "4. Pre-download Phi-4 (bf16, ~28 GB)"

# bf16 checkpoint: loads on Ampere (A100). The -FP8 variant needs compute >= 8.9.
MODEL_DIR="$HOME/.cache/huggingface/hub/models--microsoft--Phi-4"
...
3. Run the pipeline (the ~28 GB model loads into VRAM on the first call, ~minutes):
   nohup ./rad orchestrator > .rad/orchestrator-\$(date +%Y%m%d-%H%M%S).log 2>&1 &
   tail -F .rad/orchestrator-*.log
