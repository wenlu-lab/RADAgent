#!/usr/bin/env python3
"""
RAD Pipeline Token Tracker — standalone utility for all token counting and tracking.

Usage:
  python3 00-scripts/rad_token_tracker.py --init                          # Create/reset timing CSV
  python3 00-scripts/rad_token_tracker.py --record-step <N> <TITLE> <STATUS> <START> <END>  # Record step timing + tokens
  python3 00-scripts/rad_token_tracker.py --summary                       # Full token/timing report
  python3 00-scripts/rad_token_tracker.py --newest-subagent               # Tokens from most recent subagent
  python3 00-scripts/rad_token_tracker.py --main-only                     # Orchestrator (Opus) tokens only
  python3 00-scripts/rad_token_tracker.py --cumulative                    # Main + all subagents combined
...
TIMING_CSV = "rad-pipeline-timing.csv"
...
def find_project_dir():
    ...
    New (open-source agent runtime): <cwd>/.rad/transcripts/
    Legacy (Claude Code): ~/.claude/projects/<slug>/

    Prefer .rad/transcripts/ if it exists; fall back to Claude Code paths.
    """
    cwd = os.getcwd()
    local = os.path.join(cwd, ".rad", "transcripts")
...
def cmd_summary():
    if not os.path.isfile(TIMING_CSV):
        print("No timing data found (rad-pipeline-timing.csv missing).")
        return
