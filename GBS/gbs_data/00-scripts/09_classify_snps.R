...
mean_Fis <- mean(d$Fis)
sd_Fis <- sd(d$Fis)
mean_Fis_plus2sd <- mean_Fis + 2 * sd_Fis
mean_Fis_minus2sd <- mean_Fis - 2 * sd_Fis
print(paste0(("Mean Fis: "), mean_Fis, ( "Mean Fis +- 2sd: "), mean_Fis_minus2sd, ( "-"), mean_Fis_plus2sd))
...
mean_depth <- mean(d$MedCovHom)
sd_depth <- sd(d$MedCovHom)
mean_depth_within2sd <- mean_depth + 2 * sd_depth
print(paste0(("Mean depth hom: "), mean_depth, ( "Mean depth hom + 2sd: "), mean_depth_within2sd))

mean_depth_het <- mean(d$MedCovHet)
sd_depth_het <- sd(d$MedCovHet)
mean_depth_within2sd_het <- mean_depth_het + 2 * sd_depth_het
print(paste0(("Mean depth het: "), mean_depth_het, ( "Mean depth het + 2sd: "), mean_depth_within2sd_het))

# Loci with high coverage
d$Color[d$MedCovHom > mean_depth_within2sd | d$MedCovHet > mean_depth_within2sd_het] = highcov
