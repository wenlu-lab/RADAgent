...
process_radtags \
    -i gzfastq \
    -1 02-raw/"$LANE"".fastq.gz" \
    -2 02-raw/$(echo "$LANE" | perl -pe 's/_1/_2/')".fastq.gz" \
    -o 03-samples/"$LANE" \
    -c -r -t "$TRIM_LENGTH" \
    -q -s 0 \
    -E phred33 \
...
cp 03-samples/"$LANE"/process_radtags.02-raw.log 10-log_files/02_process_radtags_2_enzymes_"$LANE".log
