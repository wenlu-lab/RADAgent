---
name: gbs-6-gstacks
description: Run GBS pipeline Step 6 (gstacks genotyping) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# GBS Pipeline — Step 6: gstacks Reference-Based Genotyping (Autonomous)

You are running GBS pipeline Step 6 directly on the server. All paths are relative to the project root (`gbs_data/`). This skill is **fully autonomous** — it launches gstacks, monitors with adaptive polling, and verifies output.
...
1. **BAM files exist**: `ls 04-all_samples/*.sorted.bam | wc -l` → EXPECTED_SAMPLES. Must be > 0
   - FAIL → **stop**: "Step 5 not completed. Run `/gbs-5-bwa` first."
