---
name: gbs-4-popmap
description: Run GBS pipeline Step 4 (prepare population map) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# GBS Pipeline — Step 4: Prepare Population Map

You are running GBS pipeline Step 4 directly on the server. All paths are relative to the project root (`gbs_data/`).
...
1. **Step 3 completed**: `ls 04-all_samples/*.1.fq.gz | wc -l` → record as EXPECTED_SAMPLES. Must be > 0
   - FAIL → **stop immediately**: "Step 3 not completed. Run `/gbs-3-rename` first."
