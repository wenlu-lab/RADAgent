...
def _find_project_dir():
    """Locate transcripts directory, preferring the new local layout."""
    cwd = Path.cwd()
    local = cwd / ".gbs" / "transcripts"
    if local.is_dir():
        return local
    # Legacy fallback
    home = Path.home()
    ...
    legacy = claude_base / slug
    if legacy.is_dir():
        return legacy
    # Pick most recent claude project as last resort
    if claude_base.is_dir():
        candidates = sorted(claude_base.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True)
        if candidates:
            return candidates[0]
    return cwd / ".gbs" / "transcripts"  # may not exist; later code handles


PROJECT_DIR = _find_project_dir()


def _is_new_layout(p):
    return p.name == "transcripts" and p.parent.name == ".gbs"


def _new_layout_session_dirs(transcripts_root):
    """In new layout, session dirs live directly under .gbs/transcripts/."""
    if not transcripts_root.is_dir():
        return []
    return sorted(
        [d for d in transcripts_root.iterdir() if d.is_dir()],
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )


def _new_layout_transcripts_in_session(session_dir):
    """Return all JSONL files (orchestrator + subagents) for a new-layout session."""
...
def find_sessions():
    ...
    if _is_new_layout(PROJECT_DIR):
        # New layout: each subdir of .gbs/transcripts/ is a session
        sessions = []
        for d in _new_layout_session_dirs(PROJECT_DIR):
            files = _new_layout_transcripts_in_session(d)
            if files:
                mtime = max(f.stat().st_mtime for f in files)
                sessions.append((d.name, mtime, len(files)))
        return sessions  # already sorted by mtime desc

    # Legacy layout: look for subdirs with a subagents/ dir
...
def get_session_dir(session_id=None, run_num=None):
    """Get the session directory, defaulting to the most recent.

    In the new layout, run_num selects the Nth most-recent session dir
    (1 = oldest, N = latest), matching the convention used by filter_by_run.
    """
    if _is_new_layout(PROJECT_DIR):
        if session_id:
            ...
            if not d.is_dir():
                ...
                sys.exit(1)
            return d
        session_dirs = _new_layout_session_dirs(PROJECT_DIR)
        if not session_dirs:
            print("No transcripts found in .gbs/transcripts/.")
            sys.exit(1)
        if run_num is not None:
            # run_num is 1-based, most-recent-last (to match legacy run numbering)
            # session_dirs is sorted most-recent-first, so index from the end
            idx = len(session_dirs) - run_num
            if idx < 0 or idx >= len(session_dirs):
                print(f"Error: --run {run_num} out of range (1..{len(session_dirs)}).")
                sys.exit(1)
            return session_dirs[idx]
        return session_dirs[0]  # most recent
...
def classify_transcripts(session_dir: Path):
    ...
    # New layout: transcripts may live directly in the session dir or under subagents/
    if _is_new_layout(PROJECT_DIR):
        raw_files = _new_layout_transcripts_in_session(session_dir)
        files = sorted(raw_files, key=lambda f: f.stat().st_mtime)
    else:
...
def cmd_list(args):
    ...
    run_num_arg = getattr(args, "run", None)

    if _is_new_layout(PROJECT_DIR):
        # In new layout, --run N selects which session dir (pipeline run) to inspect
        session_dirs = _new_layout_session_dirs(PROJECT_DIR)
        if not session_dirs:
            print("No transcripts found in .gbs/transcripts/.")
            return
        total_runs = len(session_dirs)
        if run_num_arg is not None:
            idx = len(session_dirs) - run_num_arg
            if idx < 0 or idx >= len(session_dirs):
                print(f"Error: --run {run_num_arg} out of range (1..{total_runs}).")
                return
            ...
            run_num = total_runs
        transcripts, _ = classify_transcripts(session_dir)
    ...
    if run_num_arg is not None or total_runs > 1:
        filtered, run_num = filter_by_run(transcripts, total_runs, run_num_arg)
    else:
        filtered = transcripts
        run_num = 1
...
def cmd_show(args):
    ...
    if _is_new_layout(PROJECT_DIR):
        # In new layout, --run N selects which session dir to read
        session_dirs = _new_layout_session_dirs(PROJECT_DIR)
        if not session_dirs:
            print("No transcripts found in .gbs/transcripts/.")
            return
        ...
        session_dir = get_session_dir(getattr(args, "session_id", None))
        transcripts, total_runs = classify_transcripts(session_dir)
