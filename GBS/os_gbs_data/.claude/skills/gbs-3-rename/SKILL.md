---
name: gbs-3-rename
description: Run GBS pipeline Step 3 (rename samples) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# GBS Pipeline — Step 3: Rename Samples

You are running GBS pipeline Step 3 directly on the server. All paths are relative to the project root (`gbs_data/`).
...
1. **Step 2 completed**: `ls -d 03-samples/*/ | wc -l` → record as EXPECTED_LANES. Must be > 0. Spot-check one lane: `ls $(ls -d 03-samples/*/ | head -1)*.fq.gz | wc -l` — must have ≥ 4 files
   - FAIL → **stop immediately**: "Step 2 not completed. Run `/gbs-2-radtags` first."
