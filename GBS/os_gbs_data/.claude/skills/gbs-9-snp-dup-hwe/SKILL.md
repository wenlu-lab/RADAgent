---
name: gbs-9-snp-dup-hwe
description: Run GBS pipeline Step 9 (SNP duplication detection + HWE filtering) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# GBS Pipeline — Step 9: SNP Duplication Detection + HWE Filtering

You are running GBS pipeline Step 9 directly on the server. All paths are relative to the project root (`gbs_data/`).
...
1. **Input VCF exists**: `test -f filtered_m4_p80_x0_S1_chr.recode.vcf && echo OK`. Count SNPs: `grep -cv '^#' filtered_m4_p80_x0_S1_chr.recode.vcf` → INPUT_SNPS
   - FAIL → **stop**: "Step 8 not completed. Run `/gbs-8-vcf-filter` first."
