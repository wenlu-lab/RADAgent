---
name: gbs-8-vcf-filter
description: Run GBS pipeline Step 8 (VCF filtering + chromosome selection) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# GBS Pipeline — Step 8: VCF Filtering + Chromosome Selection

You are running GBS pipeline Step 8 directly on the server. All paths are relative to the project root (`gbs_data/`).
...
1. **populations VCF exists**: `test -f 05-stacks/populations.snps.vcf && echo OK`
   - FAIL → **stop**: "Step 7 not completed. Run `/gbs-7-populations` first."
