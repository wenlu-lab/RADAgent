---
name: gbs-12-maf-filter
description: Run GBS pipeline Step 12 (MAF 0.1 filter) on the server
context: fork
model: sonnet
allowed-tools: Bash, Read, Grep, Glob
---

# GBS Pipeline — Step 12: MAF 0.1 Filter

You are running GBS pipeline Step 12 directly on the server. All paths are relative to the project root (`gbs_data/`).
...
1. **Input VCF exists**: `test -f filtered_m4_p80_x0_S1_chr.singleton.hwe.maf01.LD.no_AT_CG.vcf && echo OK`. Count SNPs: `grep -cv '^#' filtered_m4_p80_x0_S1_chr.singleton.hwe.maf01.LD.no_AT_CG.vcf` → INPUT_SNPS
   - FAIL → **stop**: "Step 11 not completed. Run `/gbs-11-remove-atgc` first."
