"""Pure (UI-free) parser for GBS pipeline run transcripts under .gbs/.

Reads the JSONL transcripts written by agent/gbs/transcript.py plus the
...
_SKILL_STEP_RE = re.compile(r"gbs-(\d+)-")
...
def _index_logs(gbs_dir: Path) -> dict[str, Path]:
    """Map session_id -> orchestrator-*.log by scanning for 'Session: <id>'."""
    out: dict[str, Path] = {}
    for log in sorted(gbs_dir.glob("orchestrator-*.log")):
        try:
...
def _classify_session(session_dir: Path, log_index: dict[str, Path]) -> Run:
    ...
    for i in range(n):
        ...
        if sub:
            ...
            rows, events = [], []
            tin = tout = 0
        kind = "debugger" if skill_name == "gbs-debugger" else "step"
...
def load_runs(gbs_dir) -> list[Run]:
    """Load all runs under <gbs_dir>/transcripts/, most-recent first."""
    gbs_dir = Path(gbs_dir)
    troot = gbs_dir / "transcripts"
    if not troot.is_dir():
        return []
    log_index = _index_logs(gbs_dir)
    pairs = []
